...
<script>
import Range from './js/ueditor/Range.js';
export default {
  name: 'App',
  data() {
    return {}
  ...
  methods: {
    test(e) {
      console.log(document.querySelectorAll(`[data-mark=${e.target.dataset.mark}]`))
    },
    reset() {
      this.range.removeInlineStyle('i','nite-writer-pen')
    },
    /**
     * 添加下划线
     */
    ...
    replaceSelectedStrByEle(className) {
      ...
      this.range = getRange();
      this.range.applyInlineStyle('i', {
        class: className,
        id: this.randomString(10)
        // 'data-mark': this.randomString(10)
      });
      this.range.select();
    },
    /**
